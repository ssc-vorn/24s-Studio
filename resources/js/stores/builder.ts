...
export const useBuilderStore = defineStore('builder', () => {
  ...
  function snapshot(): PageSection[] {
    return clone(sections.value)
  }
  ...
  function hydrate(versionId: string, incoming: PageSection[]) {
  ...
  function setViewport(next: BuilderViewport) {
    viewport.value = next
  }
  ...
  function addSection(input: BuilderSectionInput, id: string) {
    ...
    siblings
      ...
      .forEach((section) => { section.position += 1 })
    ...
    next.push({
      ...
      styles: clone(input.styles ?? {}),
      responsive: clone(input.responsive ?? {}),
      animation: clone(input.animation ?? {}),
  ...
  function reorder(ids: string[], parentId: string | null = null) {
    ...
    next.forEach((section) => {
      if (ordered.has(section.id)) {
        section.parent_id = parentId
        section.position = ordered.get(section.id)!
      }
    })
    commit(next)
  }
  ...
  function markSaveError(message: string) {
