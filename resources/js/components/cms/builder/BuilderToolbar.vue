...
import { ArrowLeft, Check, Eye, Redo2, Send, Undo2, Upload } from 'lucide-vue-next'
...
<template>
  <header class="flex min-h-16 items-center justify-between gap-4 border-b border-slate-200 bg-white px-4">
    <div class="flex min-w-0 items-center gap-3">
      ...
      <div class="min-w-0">
        ...
        <div class="flex items-center gap-2 text-xs text-slate-500">
          ...
          <span class="rounded-full bg-slate-100 px-2 py-0.5 font-medium text-slate-700">{{ workflowLabel(status) }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="flex items-center gap-2">
      ...
      <button v-if="status === 'draft'" type="button" :disabled="workflowBusy || !canSubmitReview" class="inline-flex h-9 items-center gap-2 rounded-lg border border-slate-200 px-3 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50" @click="emit('submit-review')">
        <Send class="h-4 w-4" /> {{ workflowBusy ? 'Submitting…' : 'Submit for review' }}
      </button>
      <button v-else-if="status === 'review'" type="button" :disabled="workflowBusy || !canApprove" class="inline-flex h-9 items-center gap-2 rounded-lg border border-slate-200 px-3 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50" @click="emit('approve')">
        <Check class="h-4 w-4" /> {{ workflowBusy ? 'Approving…' : 'Approve' }}
      </button>
      <button v-else-if="status === 'approved'" type="button" :disabled="workflowBusy || !canPublish" class="inline-flex h-9 items-center gap-2 rounded-lg bg-slate-900 px-3 text-sm font-medium text-white hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50" @click="emit('publish')">
        <Upload class="h-4 w-4" /> {{ workflowBusy ? 'Publishing…' : 'Publish' }}
      </button>
    </div>
  </header>
</template>
