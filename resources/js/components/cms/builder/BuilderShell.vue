...
const ensureSaved = async () => {
  workflowError.value = ''
  await persist()
  if (builder.dirty || builder.saveState === 'saving' || builder.saveState === 'error' || builder.saveState === 'conflict') throw new Error('Please resolve the current save state before changing the version workflow.')
}
const runWorkflow = async (action: () => Promise<void>, nextStatus: string) => {
  if (workflowBusy.value) return
  workflowBusy.value = true
  workflowError.value = ''
  try { await ensureSaved(); await action(); status.value = nextStatus } catch (error) { workflowError.value = error instanceof Error ? error.message : 'Workflow action failed.' } finally { workflowBusy.value = false }
}
...
const restored = (version: { id: string }) => {
  historyOpen.value = false
  router.visit(`/admin/cms/organizations/${props.organization.id}/pages/${props.page.id}/builder/${version.id}`)
}
</script>

<template>
  <div class="flex h-screen min-h-[620px] flex-col overflow-hidden bg-slate-100 text-slate-900">
    <BuilderToolbar :title="page.title" :version="version.version" :status="status" :viewport="builder.viewport" :can-undo="builder.canUndo" :can-redo="builder.canRedo" :save-state="builder.saveState" :dirty="builder.dirty" :workflow-busy="workflowBusy" :can-submit-review="canSubmitReview" :can-approve="canApprove" :can-publish="canPublish" @back="goBack" @update:viewport="(value: BuilderViewport) => builder.setViewport(value)" @undo="builder.undo" @redo="builder.redo" @preview="preview" @history="historyOpen = true" @submit-review="submitReview" @approve="approve" @publish="publish" />
    ...
    <VersionHistoryPanel v-if="historyOpen" :organization-id="organization.id" :page-id="page.id" :current-version-id="version.id" :current-version="version.version" @close="historyOpen = false" @restored="restored" />
  </div>
</template>
