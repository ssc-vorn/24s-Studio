...
const load = async () => {
  ...
  try { const response = await mediaApi.list(props.organizationId, { search: query.value || undefined, per_page: 48 }); assets.value = response.data }
        ...
        <div class="min-h-0 flex-1 overflow-y-auto p-4">
          ...
          <div v-else-if="filtered.length" class="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
            <button v-for="item in filtered" :key="item.id" type="button" class="group overflow-hidden rounded-lg border text-left" :class="item.id === selectedId ? 'border-slate-900 ring-2 ring-slate-900/10' : 'border-slate-200'" @click="emit('select', item)">
              <div class="aspect-[4/3] bg-slate-100"><img v-if="item.url" :src="item.url" :alt="item.alt ?? item.filename" class="h-full w-full object-cover transition group-hover:scale-[1.02]" /><div v-else class="flex h-full items-center justify-center text-slate-300"><ImagePlus class="h-7 w-7" /></div></div>
